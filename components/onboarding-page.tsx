...
export function OnboardingPage({ onFileProcessed }: OnboardingPageProps) {
  const [isDragOver, setIsDragOver] = useState(false)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  
  // Hooks para manejar upload y procesamiento
  const fileUpload = useFileUpload()
  const fileProcessing = useFileProcessing()

  // Efecto para procesar archivo automáticamente después del upload
  useEffect(() => {
    if (fileUpload.uploadedFile && !fileProcessing.isProcessing) {
      fileProcessing.processFile(fileUpload.uploadedFile.file)
    }
  }, [fileUpload.uploadedFile, fileProcessing.isProcessing])

  // Efecto para notificar cuando el procesamiento esté completo
  useEffect(() => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    // El useFileUpload hook maneja toda la lógica de subida
  ...
  const handleGenerateDashboard = () => {
    if (fileProcessing.result) {
      onFileProcessed({
        ...
        schema: fileProcessing.result.schema,
        ...
        <Card className="shadow-xl border border-gray-200">
          <CardHeader className="text-center">
            ...
            <p className="text-gray-600">
              Arrastra tu archivo CSV o Excel aquí para empezar.
            </p>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
                ...
                {!selectedFile ? (
                  <>
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Soportamos archivos CSV y Excel (.xlsx, .xls) - Máximo 10MB
                    </p>
                  </>
                ) : (
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-orange-600 flex items-center gap-2">
                    <Loader2 className="h-4 w-4 animate-spin" />
                    Analizando estructura y detectando tipos...
                  </span>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-green-600 text-sm">
                      {fileProcessing.result?.stats.totalRows} filas, {fileProcessing.result?.stats.totalColumns} columnas detectadas
                    </p>
                    <p className="text-green-600 text-xs mt-1">
                      Tipos detectados automáticamente con IA
                    </p>
                  </div>
                </div>
            ...
            >
              {isCompleted ? '🚀 Generar Dashboard Inteligente' : 'Generar Dashboard'}
            </Button>
          </CardContent>
        </Card>
      </div>
